...
def _collect_step_states(env, initial_state, fp16_chunks, chunk_states, num_steps_wait):
    env.reset()
    obs = env.set_init_state(initial_state)
    done = False
    ...
    for chunk_idx, chunk in enumerate(fp16_chunks):
        ...
        for step_idx, a in enumerate(chunk):
            step_states.append({
                "chunk_idx": chunk_idx,
                "step_idx": step_idx,
                "mujoco_state": {
                    "time": float(env.sim.get_state().time),
                    "qpos": [float(x) for x in env.sim.get_state().qpos],
                    "qvel": [float(x) for x in env.sim.get_state().qvel],
                },
            })
            try:
                obs, reward, done, info = env.step(a)
...
def _run_with_chunk_perturbation(env, initial_state, fp16_chunks, w4a4_chunks,
...
def _run_with_step_perturbation(env, initial_state, fp16_chunks, w4a4_chunks,
                                  chunk_states, num_steps_wait, chunk_perturb,
                                  step_perturb, task_description, all_step_states,
                                  fp16_client, args):
    env.reset()
    obs = env.set_init_state(initial_state)
    done = False
    ...
    flat_fp16 = []
    for ci, chunk in enumerate(fp16_chunks):
        for si, a in enumerate(chunk):
            flat_fp16.append((ci, si, a))

    global_idx = None
    for idx, (ci, si, _) in enumerate(flat_fp16):
        if ci == chunk_perturb and si == step_perturb:
            global_idx = idx
            break

    if global_idx is None or global_idx >= len(all_step_states):
        return np.zeros(3), False, replay_images

    state_before = all_step_states[global_idx]["mujoco_state"]
    mj = env.sim.get_state()
    mj.time = state_before["time"]
    mj.qpos[:] = state_before["qpos"]
    mj.qvel[:] = state_before["qvel"]
    env.sim.set_state(mj)
    env.sim.forward()
    obs = env.get_observation()
    ...
    prev_eef = _get_eef_pos(env, obs)
    for idx in range(global_idx + 1, len(flat_fp16)):
        ci, si, a = flat_fp16[idx]
        try:
            obs, reward, done, info = env.step(a)
        except ValueError:
            done = True
...
def _run_perturbation(args: Args, fp16_client) -> None:
    ...
    for traj_path in tqdm.tqdm(combined_paths):
        ...
        if args.mode == 3:
            ...
            for k in range(num_chunks):
                ...
                try:
                    perturb_eef, perturb_success, perturb_images = _run_with_chunk_perturbation(
                        env_k, initial_state, fp16_chunks, w4a4_chunks,
            ...
            logging.info(f"    Collecting step states for {traj_name}...")
            env_collect, _ = _get_libero_env(task, LIBERO_ENV_RESOLUTION, args.seed)
            all_step_states = _collect_step_states(
                env_collect, initial_state, fp16_chunks, chunk_states, args.num_steps_wait
            )
            env_collect.close()
            logging.info(f"    Collected {len(all_step_states)} step states")
                ...
                for s in range(num_steps):
                    ...
                    try:
                        perturb_eef, perturb_success, perturb_images = _run_with_step_perturbation(
                            ...
                            task_description, all_step_states, fp16_client, args
                        )
                    finally:
                        env_ks.close()
